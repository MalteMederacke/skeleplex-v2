"""Parallel worker for Fusion 1.3: Process Scale Map — one batch of chunks per SLURM task."""
...
img_zarr = zarr.open(INPUT_IMAGE_PATH, mode="r")
scale_zarr = zarr.open(SCALE_MAP_PATH, mode="r")
radius_zarr = zarr.open(RADIUS_MAP_PATH, mode="r")
output_zarr = zarr.open(SCALE_MAP_PROCESSED_PATH, mode="a")

for chunk_id in range(start, end):
    expanded, core_in_result, core_out, _ = get_slices_for_chunk(job_df, chunk_id)
    result = scale_map_processing_gpu(
        img_zarr[expanded], scale_zarr[expanded], radius_zarr[expanded]
    )
